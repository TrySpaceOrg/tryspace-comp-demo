...
int32 DEMO_AppInit(void)
{
    ...
    */
    ...
    DEMO_AppData.HkTelemetryPkt.DeviceHK.DeviceCounter = 0;
    DEMO_AppData.HkTelemetryPkt.DeviceHK.DeviceConfig  = 0;
    DEMO_AppData.HkTelemetryPkt.DeviceHK.DeviceStatus  = 0;
...
void DEMO_ReportDeviceTelemetry(void)
{
    ...
    if (DEMO_AppData.HkTelemetryPkt.DeviceEnabled == DEMO_DEVICE_ENABLED)
    {
        ...
        if (DEMO_AppData.HkTelemetryPkt.DeviceHK.DeviceStatus != 0)
        {
            /* Any bit is an error, halting communication until device power cycled */
            DEMO_Disable();

            /* Send device status error to the console */
            CFE_EVS_SendEvent(DEMO_REQ_DATA_STATUS_ERR_EID, CFE_EVS_EventType_ERROR,
                              "DEMO: Request device data reported status error %d",
                              DEMO_AppData.HkTelemetryPkt.DeviceHK.DeviceStatus);
        }
    }
